...
- **전체 기록 검색 (Full-text Search):** (✅ 완료 - 2025-11-05)
  - **문제점:** 채팅 기록이 많아지면 특정 내용을 찾기 위해 모든 대화를 일일이 열어봐야 하는 불편함이 있습니다.
  - **개선 제안:** 채팅 기록 화면에 검색창을 추가하여, 모든 세션의 제목과 메시지 내용을 대상으로 하는 전문(Full-text) 검색 기능을 구현합니다.
  - **구현 내용:**
    - **검색 라이브러리 도입:** 가볍고 빠른 클라이언트 사이드 검색 라이브러리인 `fuse.js`를 도입했습니다.
    - **검색 UI 구현:** `HistoryView` 컴포넌트 헤더에 검색 아이콘과 함께 검색 입력창을 추가했습니다.
    - **검색 로직 구현:** `useMemo`를 사용하여 `fuse.js` 인스턴스를 효율적으로 생성하고, 사용자의 검색어 입력에 따라 실시간으로 세션 목록을 필터링하도록 구현했습니다. 검색 대상은 세션의 `title`과 각 메시지의 `content`를 포함합니다.
    - **결과 표시:** 검색 결과가 없을 경우 "검색 결과가 없습니다"라는 메시지를 표시하여 사용자에게 명확한 피드백을 제공합니다.
  - **구현 위치:**
    - `package.json` (dependency 추가)
    - `src/sidepanel/components/HistoryView.tsx`
